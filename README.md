...
- [x] 句読点確定
- [x] Windows 11 ネイティブ UI の設定画面、候補ウィンドウ、入力モードインジケータ
...
Zenzaiを有効にして、Vulkan backend で変換精度を向上させます。

### バックエンド
Vulkan backend のみをサポートしています。

- **Vulkan**: GPUのドライバーに標準で含まれているため、追加のインストールは不要です。CPU backend は提供しません。
...
ビルド・配布・起動経路の UI 実体は `settings.exe` と `ui.exe` だけです。
